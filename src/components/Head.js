    ...
    <div className='grid grid-flow-col p-2 m-2 shadow-lg'>
      ...
      <div className='flex col-span-10 justify-center h-10'>
        <input placeholder='Search'
            ...
            type='text'
            onFocus={() => setDisplaySugesstions(true)}
            onBlur={() => setDisplaySugesstions(false)}>
        </input>
        <button className='w-14 bg-gray-200 border border-black rounded-r-2xl'>🔍</button>

        {displaySugesstions && 
          <div className='absolute bg-white my-10 w-[440px] mr-24 mx-2 rounded-lg border border-gray-100'>
            <ul>
              {searchSugesstions.map((value) => (
                  <li key={value} className='py-2 px-4 border border-gray-100 hover:bg-gray-100'>🔍 {value}</li>
              ))}
            </ul>
          </div>
        }
